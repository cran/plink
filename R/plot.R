	...
	if (dimensions==1) {
		...
		if (type %in% c("vectorplot1","vectorplot2","vectorplot3")) {
			if (dimensions<7) {
				...
				# Identify the dimensions of panels
				pan <- matrix(c(rep(1:6,c(2,2,5,7,9,11)),rep(1:6,c(1,5,6,8,10,6))),36,2)
				par(mfrow=c(pan[panels,1],pan[panels,2]))
				
				if (!missing(items)) {
					a.all <- as.matrix(x@pars$a)[items,]
					b <- as.matrix(as.matrix(x@pars$b)[items,])
				} else {
					a.all <- as.matrix(x@pars$a)
					b <- as.matrix(x@pars$b)
				}
				if (is.vector(a.all) & length(unlist(x@items))==1) a.all <- t(a.all)
				for (i in 1:(dimensions-1)) {
					for (j in (i+1):dimensions) {
						a <- a.all[,c(i,j)]
						if (is.vector(a) & length(unlist(x@items))==1) a <- t(a)
						...
						if (is.vector(dcos) & length(unlist(x@items))==1) dcos <- t(dcos)
						mdiff <- -apply(b,1,sum,na.rm=T)/(den.con*mdisc)
						item.names <- 1:length(mdisc)
						...
						if (item.nums==TRUE) text(x3,y3,item.names, col=2)
						text(xmm[2],0.1*ymm[2], eval(parse(text=paste("expression(paste(theta[",i,"]))",sep=""))),cex=1.2)
						text(0.1*xmm[2],ymm[2], eval(parse(text=paste("expression(paste(theta[",j,"]))",sep=""))),cex=1.2)
					}
				}
			} else {
...
plot.sep.pars <- function(x, y, ...) {
	...
	if (length(dots$type)) {
		if (dots$type %in% c("vectoplot1","vectorplot2","vectorplot3")) {
			theta <- 1
		}
	}
	...
	tmp <- mixed(x, theta=theta, catprob=catprob, location=x@location, incorrect=incorrect, D.drm=D.drm, D.gpcm=D.gpcm, D.grm=D.grm)
	plot(tmp, ...)
}
